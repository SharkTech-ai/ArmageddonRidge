...
public partial class TankReadout
{
    ...
    [Parameter] public int? DisplayHealth { get; set; }
    [Parameter] public float? DisplayShield { get; set; }
    ...
    private int CurrentHealth => Math.Max(0, DisplayHealth ?? Tank.Health);

    private string HealthWidth => $"{Math.Clamp(CurrentHealth / (float)Math.Max(Tank.MaxHealth, 1), 0, 1) * 100:0}%";

    private string ShieldWidth => $"{Math.Clamp(MathF.Max(0, DisplayShield ?? Tank.Shield) / 120f, 0, 1) * 100:0}%";
}
